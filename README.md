...
- Detect direction drift only for tasks explicitly created from or bound to a project's declared next action. Ordinary linked tasks can be parallel work or sub-steps and never manufacture a calibration chore merely because their title differs. A real bound-direction conflict can adopt the live task or retain the declared direction, with both choices audited.
...
- Open a compact project workbench centered on today's tasks and Codex conversations. It shows one live-state badge and one-line next-action context; command cards appear only for a real actionable exception, empty decision history stays hidden, and detailed project fields expand only when the user chooses **Edit**.
