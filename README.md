...
- Process that review queue in calm batches of up to five projects from the home workspace: inspect the saved objective, stage, health, cadence, and next action; confirm deliberately, defer without mutating data, or open the full project workbench to correct the decision first. The home card keeps the full backlog visible and separates first reviews from overdue reviews.
...
- Keep past planned work visible as a calm planning-backlog decision instead of silently rolling it forward or letting it disappear into history. Review the queue from the home decision panel or Ctrl+K, then deliberately reschedule, edit, or leave each item unchanged.
