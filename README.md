...
- Process the **Management Confirmation** queue in calm batches of up to five projects from the home workspace: inspect the saved objective, stage, health, cadence, and next action; confirm deliberately, defer without mutating data, or open the full project workbench to correct the decision first. Inspecting the workbench no longer abandons the batch: closing it returns to the same review item, reloads the current project record, and advances automatically when the issue was resolved there. Project rows distinguish missing governance, an initial baseline, and an overdue cadence review; items already owned by execution-alignment or lifecycle decisions are explicitly shown as routed instead of being counted twice in the direct queue.
