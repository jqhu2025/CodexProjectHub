...
- Rank those competing queues into one **Next Management Decision**: confirmed risk, today's WIP overload, execution-direction drift, missing next actions, focus-capacity mismatch, strategic-focus commitment, scheduled review, and lifecycle calibration are evaluated in a stable order. The home workspace shows only the strongest current recommendation and routes it to the existing auditable workflow, reducing dashboard scanning without hiding the underlying counts.
