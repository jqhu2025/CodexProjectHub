...
- Carry that distinction through the project cockpit instead of merging it into a generic “focus” count. The current view reports strategic focus against its configured capacity and actual execution separately, explains focus/execution misalignment in neutral language, and lets users open focus calibration or filter to live work directly from the cockpit. Those global counts remain stable when the category map is filtered, so a click always opens the same portfolio represented by the number. A project can still display both badges at once; risk remains the first sort key, followed by deliberate strategic focus and then merely busy work, so activity is never mistaken for importance. The cockpit also shows a mutually exclusive distribution of primary project decisions—risk response, execution alignment, lifecycle calibration, missing next action, focus commitment, or management confirmation—and every nonzero count opens the exact existing workflow instead of hiding work behind an opaque “other decisions” total.
- Keep a configurable one-to-nine-project focus capacity. The home decision queue shows focus usage and real parallel work together, while a dedicated calibration panel defaults to the projects with actual live execution and can switch to the selected-focus set or the complete active portfolio on demand. Promoting or demoting a project always uses the normal project-decision audit trail; crossing the configured capacity requires an explicit impact confirmation that states the before/after counts and never silently removes another priority.
- Frame live work as strategic candidates rather than implied priorities: when execution exceeds the available focus slots, the dashboard states how many projects may be selected, while the calibration panel compares each candidate's objective, health, next action, active-task count, and running Codex evidence before a human makes the trade-off.
- Turn strategic focus into an explicit execution commitment: the focus panel shows each project's stage, health, declared next action, and whether that action is already on the task board. A focus item with no other live work can add its next action to today's plan in one click, while existing tasks or a running Codex conversation suppress duplicate scheduling prompts.
- Surface a compact home decision queue for focus capacity, confirmed risk or blockage, neutral review reminders, and missing-next-action projects; every card opens the exact decision flow, and review debt is never presented as operational risk. Project-level queues use explicit primary ownership—risk response, execution alignment, lifecycle calibration, missing next action, strategic-focus commitment, then routine review—so one project never inflates several actionable counts. A card whose raw candidates were all claimed by a higher-priority decision explains where they went instead of appearing mysteriously empty.
- Rank those competing queues into one **Next Management Decision**: confirmed risk, today's WIP overload, execution-direction drift, missing completion evidence, historical plan backlog, missing next actions, focus-capacity mismatch, strategic-focus commitment, scheduled review, and lifecycle calibration are evaluated in a stable order. The home workspace shows the strongest current recommendation, a verifiable completion criterion, and a compact but complete summary of every remaining decision queue before routing to the existing auditable workflow.
...
- Run a portfolio-wide Codex governance pass over selected projects with missing decisions. Analysis is sequential and read-only, every proposed field is reviewed before application, existing human decisions are never overwritten, and gaps are rechecked at write time to reject stale suggestions. A project cannot establish or refresh its review baseline while required governance fields remain missing; the guided review routes incomplete local projects directly into Codex-assisted completion, and a fully applied suggestion starts the review cadence.
...
- Open a single project workbench that resolves one primary command before exposing the full form. Risk, execution-direction drift, lifecycle calibration, a missing next action, focus commitment, and routine review follow the same portfolio routing order, while today's task mix, running Codex conversations, latest activity, objective, and next action remain visible as decision evidence. The project map and sortable category list reuse that exact command instead of showing a lower-priority raw review state, so the instruction seen before opening a project matches the action inside it. Detailed metadata expands only on demand, leaving the default view focused on action, tasks, and conversations.
...
- Confirm a project's current state without inventing a field change. Every active project first establishes a truthful, comparable review baseline across objective, acceptance criteria, stage, health, blocker, and next action; reviewed projects then use a 3/7/14-day cadence for focus/normal/later priorities and re-enter a neutral review queue when due. The next confirmation highlights only the fields that actually changed since that baseline, while legacy records begin comparison at their next explicit confirmation instead of receiving reconstructed history.
- Keep that review baseline truthful: full editor confirmation, complete creation, or reviewed Codex governance may refresh it; narrow actions such as changing strategic focus, lifecycle calibration, category placement, alignment, or undo remain audited without resetting review debt.
- Process the **Management Confirmation** queue in calm batches of up to five projects from the home workspace. A compact live batch summary distinguishes missing governance, initial baselines, and overdue cadence reviews before the first decision, then updates as each item is handled. Missing fields are handled first, measured cadence debt is ordered from longest overdue to newest due, and first-time baselines remain neutral instead of being mislabelled as overdue. Inspect the saved objective, stage, health, cadence, and next action; confirm deliberately, defer without mutating data, or open the full project workbench to correct the decision first. Inspecting the workbench no longer abandons the batch: closing it returns to the same review item, reloads the current project record, and advances automatically when the issue was resolved there. Items already owned by risk response, execution alignment, lifecycle calibration, missing-next-action, or focus-commitment decisions are explicitly shown as routed instead of being counted twice in the direct queue.
- Name a homogeneous confirmation queue by its real purpose—**Data Completion**, **First Baseline**, or **Due Review**—instead of hiding all three behind a generic approval count. Establishing the first baseline remains an explicit human decision because it becomes the comparison point for every later drift review; the application never fabricates that confirmation.
- Reduce repetitive setup without weakening governance: a review batch can establish several first baselines through one explicit confirmation only when every included project is complete, healthy, and aligned with its live work. Missing fields, risk states, due reviews, and execution divergence stay in the one-project-at-a-time flow. The batch writes an audit entry per project under one shared batch ID and offers an immediate conflict-safe undo that restores the previous review state without touching project content.
- Keep the home workload honest when those modes coexist: the confirmation card counts only projects that need an individual management judgement, names that count **Needs Your Confirmation**, and separately reports how many healthy, complete projects can share one guarded first-baseline action.
- Calibrate legacy **Needs Attention** projects with an explicit binary decision: keep the confirmed risk state or mark the project recovered. Recovery is recorded as a dedicated review-calibration decision and establishes the new review baseline in the same audited update.
- Ground every review in live evidence: show today's linked task mix, running Codex conversations, latest real activity, and declared-versus-live execution direction before confirmation. Projects with unresolved execution drift leave the general review backlog and enter the dedicated alignment workflow; a conflicting review can never establish a new baseline until that decision is acknowledged.
...
- Keep past planned work visible as a calm planning-backlog decision instead of silently rolling it forward or letting it disappear into history. Review the queue from the home decision panel or Ctrl+K, then deliberately reschedule, edit, or leave each item unchanged. A move to today offers a time-limited undo in both the review dialog and status bar, and the reversal is retained as a second auditable schedule event.
...
- Use a configurable soft WIP limit for in-progress tasks. The board shows capacity directly in the column header, never blocks a legitimate manual or Codex transition, and opens a focused review that can move idle work back to Planned while protecting tasks whose Codex conversations are still running. When capacity is exceeded, the review recommends one reversible reduction at a time from explicit evidence—runtime protection first, project priority second, and deliberate board order third—while leaving the final action under human control. Every row explains whether it is the primary reduction, a later candidate, deliberately retained, or protected by active Codex work, so the user can see why one task was chosen over another. The review states the expected capacity impact, exposes undo inside the modal where the decision is made, and records the transition as a distinct `WIP reduction` audit event.
...
- Keep completion lightweight without letting evidence disappear: a task may move to **Completed** immediately, while any item without an actual result enters the home management decision and Ctrl+K review queue until its delivery, finding, or verification is recorded. The application never fabricates that evidence.
