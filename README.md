...
- Keep the default home screen deliberately light: Codex quota and live status, yesterday's compact review, and one three-state daily task board. Portfolio calibration, review cadence, risk handling, lifecycle decisions, and data repair remain available in project-specific or on-demand workflows instead of becoming recurring dashboard chores.
...
- Process the optional **Management Confirmation** workflow in calm batches of up to five projects. A compact live batch summary distinguishes missing governance, initial baselines, and overdue cadence reviews before the first decision, then updates as each item is handled. Missing fields are handled first, measured cadence debt is ordered from longest overdue to newest due, and first-time baselines remain neutral instead of being mislabelled as overdue. Inspecting the workbench returns to the same review item and advances automatically when the issue was resolved there.
...
- When confirmation work is opened on demand, distinguish projects that need individual judgement from healthy, complete projects that can share one guarded first-baseline action.
